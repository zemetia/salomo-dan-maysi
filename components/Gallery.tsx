...
const IMAGES: ImageConfig[] = [
  { url: '/images/PMOP4036.jpg', ratio: 'portrait', offset: '-8%', rotate: -2, zIndex: 10 },
  { url: '/images/PMOP4072.jpg', ratio: 'square', offset: '12%', rotate: 3, zIndex: 20 },
  { url: '/images/PMOP4092.jpg', ratio: 'landscape', offset: '-2%', rotate: -1, zIndex: 15 },
  { url: '/images/PMOP4237.jpg', ratio: 'portrait', offset: '6%', rotate: 2, zIndex: 25 },
  { url: '/images/PMOP4303.jpg', ratio: 'square', offset: '-10%', rotate: -4, zIndex: 10 },
  { url: '/images/PMOP4431.jpg', ratio: 'portrait', offset: '8%', rotate: 1, zIndex: 30 },
  { url: '/images/PMOP4526.jpg', ratio: 'portrait', offset: '-5%', rotate: -3, zIndex: 15 },
  { url: '/images/Stry_PMOP4112.jpg', ratio: 'square', offset: '10%', rotate: 4, zIndex: 40 },
  { url: '/images/Stry_PMOP4112.jpg', ratio: 'square', offset: '10%', rotate: 4, zIndex: 40 },
];

const GalleryItem: React.FC<{ img: ImageConfig; idx: number }> = ({ img, idx }) => {
  const ratioClasses = {
  ...
  return (
    ...
    >
      <div className={`overflow-hidden bg-[#F2EFE9] ${ratioClasses[img.ratio]}`}>
        <img 
          src={img.url} 
        ...
        <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 opacity-0 group-hover:opacity-100 transition-[opacity,transform] duration-700 translate-y-4 group-hover:translate-y-0">
           <span className="font-script text-white text-5xl md:text-7xl">0{idx + 1}</span>
        </div>
      </div>
    ...
    <div ref={targetRef} className="relative h-[400vh] bg-[#F8F6F2]">
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col justify-center z-10 bg-[#F8F6F2]">
        ...
        <div className="absolute inset-0 flex items-center justify-around pointer-events-none opacity-[0.03] z-0 overflow-hidden">
           <span className="font-script text-[40vw] text-gold whitespace-nowrap select-none">Forever</span>
           <span className="font-script text-[40vw] text-gold whitespace-nowrap ml-[100vw] select-none">Journeys</span>
        </div>
        ...
        >
          <h2 className="font-script text-6xl md:text-8xl text-gold mb-3 italic leading-tight">{t.title}</h2>
        ...
        <motion.div style={{ x: particleX }} className="absolute inset-0 z-20 pointer-events-none w-[200vw]">
          {[...Array(12)].map((_, i) => (
            <div 
              key={i}
        ...
        <div className="relative h-full flex items-center overflow-visible z-10 w-max">
          ...
          <motion.div ref={ghostRef} style={{ x }} className="flex items-center h-full px-[10vw] gap-4 will-change-transform">
            {IMAGES.map((img, idx) => (
            ...
            <div className="flex-shrink-0 ml-20 md:ml-40 pr-[20vw]">
              <div className="glass-card px-16 md:px-28 py-20 md:py-32 rounded-[60px] md:rounded-[90px] border border-gold/20 shadow-2xl backdrop-blur-2xl group cursor-default text-center">
                <span className="text-gold/40 text-xs tracking-[0.8em] uppercase mb-8 block">Our Future</span>
        ...
        <div className="absolute bottom-[6dvh] md:bottom-[10dvh] left-1/2 -translate-x-1/2 flex items-center gap-8 opacity-40">
          ...
          <span className="text-[9px] md:text-[11px] uppercase tracking-[0.6em] text-taupe font-bold">Scroll to Unfold</span>
          <div className="w-12 md:w-20 h-[1px] bg-gold"></div>
        </div>
      </div>
    </div>
